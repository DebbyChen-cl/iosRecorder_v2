...
**Required companion CLI guide**: follow `.claude/commands/recorder-cli-skill.md` for every recorder CLI call, validation step, and test generation/export command.
...
`.claude/commands/recorder-cli-skill.md`. Convert the checklist into an ordered source-of-truth table: step number, intended action, semantic target, and expected verification. The recorded steps must be checked against this table before any pytest run.
...
3. Record, validate, generate, and run that case before moving to the next case.
