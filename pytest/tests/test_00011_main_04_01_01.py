...
@pytest.mark.name('00011_main_04_01_01')
def test_00011_main_04_01_01(actions: DriverActions):
    """camera - rear"""
    ...
    with step("[Action] Tap btnSettings at (36.4%, 41.2%)"):
        actions.tap_within_element(AppiumBy.ACCESSIBILITY_ID, 'btnSettings', 36.4, 41.2, container_by=AppiumBy.XPATH, container_value='//XCUIElementTypeOther[@name="LauncherProViewController"]/XCUIElementTypeScrollView', container_w=430, container_h=843)
    with step("[Action] Tap About at (90.2%, 73.9%)"):
        actions.tap_within_element(AppiumBy.ACCESSIBILITY_ID, 'About', 90.2, 73.9, container_by=AppiumBy.XPATH, container_value='//XCUIElementTypeOther[@name="photodirector.SettingPageViewController"]/XCUIElementTypeScrollView/XCUIElementTypeOther[1]/XCUIElementTypeCollectionView', container_w=430, container_h=592)
    with step("[Action] Five tap developerButton at (65.3%, 36.0%)"):
        actions.five_tap_within_element(AppiumBy.ACCESSIBILITY_ID, 'developerButton', 65.3, 36.0)
    with step("[Action] Tap Free at (44.1%, 42.9%)"):
        actions.tap_within_element(AppiumBy.ACCESSIBILITY_ID, 'Free', 44.1, 42.9, container_by=AppiumBy.XPATH, container_value='//XCUIElementTypeApplication[@name="PhotoDirector"]/XCUIElementTypeWindow/XCUIElementTypeOther[2]/XCUIElementTypeOther/XCUIElementTypeOther[1]/XCUIElementTypeOther/XCUIElementTypeOther/XCUIElementTypeOther/XCUIElementTypeOther/XCUIElementTypeOther[1]/XCUIElementTypeOther/XCUIElementTypeOther/XCUIElementTypeOther[1]/XCUIElementTypeOther/XCUIElementTypeOther/XCUIElementTypeOther/XCUIElementTypeScrollView', container_w=430, container_h=932)
    with step("[Action] Tap Pro+ at (53.5%, 59.2%)"):
        actions.tap_within_element(AppiumBy.ACCESSIBILITY_ID, 'Pro+', 53.5, 59.2, container_by=AppiumBy.XPATH, container_value='//XCUIElementTypeAlert[@name="Select an Option"]/XCUIElementTypeOther[1]/XCUIElementTypeOther/XCUIElementTypeOther[2]/XCUIElementTypeScrollView[2]', container_w=320, container_h=305)
    with step("[Action] Tap chevron.left at (30.0%, 63.9%)"):
        actions.tap_within_element(AppiumBy.ACCESSIBILITY_ID, 'chevron.left', 30.0, 63.9, container_by=AppiumBy.XPATH, container_value='//XCUIElementTypeApplication[@name="PhotoDirector"]/XCUIElementTypeWindow/XCUIElementTypeOther[2]/XCUIElementTypeOther/XCUIElementTypeOther[1]/XCUIElementTypeOther/XCUIElementTypeOther/XCUIElementTypeOther/XCUIElementTypeOther/XCUIElementTypeOther[1]/XCUIElementTypeOther/XCUIElementTypeOther/XCUIElementTypeOther[1]/XCUIElementTypeOther/XCUIElementTypeOther/XCUIElementTypeOther/XCUIElementTypeScrollView', container_w=430, container_h=932)
    with step("[Action] Tap btnBack at (39.3%, 55.3%)"):
        actions.tap_within_element(AppiumBy.ACCESSIBILITY_ID, 'btnBack', 39.3, 55.3)
    with step("[Action] Tap btnBack at (32.1%, 51.1%)"):
        actions.tap_within_element(AppiumBy.ACCESSIBILITY_ID, 'btnBack', 32.1, 51.1)
    ...
    if (not actions.tap_by_locator(AppiumBy.ACCESSIBILITY_ID, 'btnSetting')):
    ...
    if not actions.is_element_present(AppiumBy.ACCESSIBILITY_ID, 'btnShutterMode'):
        assert False, 'Shutter mode switch fail - ic mobile touch on not found'
    if not actions.is_element_present(AppiumBy.ACCESSIBILITY_ID, 'btnShutterMode'):
        assert False, 'Shutter mode switch fail - ic smile detec not found'
    if not actions.is_element_present(AppiumBy.ACCESSIBILITY_ID, 'btnShutterMode'):
        assert False, 'Shutter mode switch fail - ic shutter normal not found'
    with step('[Action] tap_night_btn'):
        assert actions.tap_by_locator(AppiumBy.ACCESSIBILITY_ID, 'btnNight')
    with step('[Action] tap_night_btn'):
        assert actions.tap_by_locator(AppiumBy.ACCESSIBILITY_ID, 'btnNight')
    with step("[Action] Tap btnFlash at (75.0%, 62.5%)"):
        actions.tap_within_element(AppiumBy.ACCESSIBILITY_ID, 'btnFlash', 75.0, 62.5)
    with step("[Verify] Capture 'flash_Step02' for GT comparison"):
        actions.capture_for_gt('flash_Step02', AppiumBy.ACCESSIBILITY_ID, 'btnFlash', threshold=0.7)
    with step("[Action] Tap btnFlash at (52.5%, 37.5%)"):
        actions.tap_within_element(AppiumBy.ACCESSIBILITY_ID, 'btnFlash', 52.5, 37.5)
    with step("[Verify] Capture 'flash_Step04' for GT comparison"):
        actions.capture_for_gt('flash_Step04', AppiumBy.ACCESSIBILITY_ID, 'btnFlash', threshold=0.7)
    with step("[Action] Tap btnFlash at (50.0%, 60.0%)"):
        actions.tap_within_element(AppiumBy.ACCESSIBILITY_ID, 'btnFlash', 50.0, 60.0)
    with step("[Verify] Capture 'flash_Step06' for GT comparison"):
        actions.capture_for_gt('flash_Step06', AppiumBy.ACCESSIBILITY_ID, 'btnFlash', threshold=0.7)
    with step("[Action] Tap btnFlash at (57.5%, 75.0%)"):
        actions.tap_within_element(AppiumBy.ACCESSIBILITY_ID, 'btnFlash', 57.5, 75.0)
    with step("[Verify] Capture 'flash_Step08' for GT comparison"):
        actions.capture_for_gt('flash_Step08', AppiumBy.ACCESSIBILITY_ID, 'btnFlash', threshold=0.7)
    ...
    with step('[Action] tap_timer_10_btn'):
        assert actions.find_element(AppiumBy.ACCESSIBILITY_ID, '10')
        assert actions.tap_by_locator(AppiumBy.ACCESSIBILITY_ID, 'btnCounter')
    with step('[Action] Back'):
        actions.tap_by_locator(AppiumBy.ACCESSIBILITY_ID, 'btnHome')
    with step("[Verify] Screenshot comparisons"):
        actions.run_screenshot_comparisons(threshold=0.7)
    with step("[Verify] test_00011 completion"):
        assert True
