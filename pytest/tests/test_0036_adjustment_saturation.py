...
@pytest.mark.name("test_adjustment_saturation")
def test_test_adjustment_saturation(actions: DriverActions):
    ...
    with step("[Action] Tap saturationColorPickButton"):
        actions.tap_by_locator(AppiumBy.ACCESSIBILITY_ID, 'saturationColorPickButton')
    ...
    with step("[Action] Tap Saturation"):
        actions.tap_by_locator(AppiumBy.ACCESSIBILITY_ID, 'Saturation')
    with step("[Verify] Saturation is not visible"):
        assert actions.verify_not_visible(AppiumBy.NAME, 'Saturation'), 'element Saturation should not be visible'
    with step("[Verify] //*[@name=\"Saturation\"] is not visible"):
        assert actions.verify_not_visible(AppiumBy.XPATH, '//*[@name="Saturation"]'), 'element //*[@name="Saturation"] should not be visible'
    with step("[Verify] //*[@label=\"Saturation\"] is not visible"):
        assert actions.verify_not_visible(AppiumBy.XPATH, '//*[@label="Saturation"]'), 'element //*[@label="Saturation"] should not be visible'
    with step("[Verify] //*[@value=\"Saturation\"] is not visible"):
        assert actions.verify_not_visible(AppiumBy.XPATH, '//*[@value="Saturation"]'), 'element //*[@value="Saturation"] should not be visible'
