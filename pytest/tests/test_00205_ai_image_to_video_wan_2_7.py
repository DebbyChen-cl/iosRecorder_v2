...
@pytest.mark.name("00205_ai_image_to_video_wan_2_7")
def test_00205_ai_image_to_video_wan_2_7(actions: DriverActions):
    ...
    with step(f"[Action] Open Custom and select {MODEL}"):
        ...
        actions.tap_by_locator(
            ...
            '//XCUIElementTypeStaticText[@name="Model"]'
            '/following-sibling::XCUIElementTypeOther[1]/XCUIElementTypeButton',
        )
    ...
    with step("[Verify] Standard and Pro quality are listed and selectable"):
        ...
        actions.tap_by_locator(AppiumBy.ACCESSIBILITY_ID, pro_id)
        assert actions.find_element(
