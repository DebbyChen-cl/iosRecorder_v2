...
def test_00192_UGC_02(actions: DriverActions):
    ...
    def _search_like_count_value(expected_like_count=None):
        if expected_like_count is None:
            return _get_like_count()
        template = _get_template_by_like_count(expected_like_count)
        return expected_like_count if template is not None else -1
    ...
    def _get_template_by_like_count(like_count):
        ...
        for cell in cells:
            ...
            for text_element in text_elements:
                raw_text = (text_element.get_attribute('label') or text_element.get_attribute('value') or text_element.get_attribute('name') or '')
    ...
    with step('Verify like count on the feed card matches step 5'):
        like_count = _search_like_count_value(like_after)
    ...
    with step('Verify entered Feedback page'):
        with step('[Action] verify_feedback_page'):
            assert actions.is_element_present(AppiumBy.ACCESSIBILITY_ID, 'lblTitle', timeout=8), 'Feedback page not displayed after tapping Report'
