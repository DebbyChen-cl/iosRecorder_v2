...
@pytest.mark.name('00167_image_to_video_kling_25_turbo')
def test_00167_image_to_video_kling_25_turbo(actions: DriverActions):
    """1. Launch ITV, import city photo from Sample Photos, select Custom style"""

    with step('Dismiss blocking promotion if present'):
        with step('[Action] tap_phd_btn'):
            if actions.is_element_present(AppiumBy.ACCESSIBILITY_ID, 'btnClose', timeout=3):
    ...
    with step('Tap Custom style'):
        with step('[Action] tap_phd_btn'):
            assert actions.tap_by_locator(AppiumBy.ACCESSIBILITY_ID, 'CMS-local_custom')
    ...
    with step('Select Kling 2.5 turbo'):
        with step('[Action] tap_phd_element'):
            assert actions.tap_by_locator(AppiumBy.ACCESSIBILITY_ID, 'Kling 2.5 Turbo')
    with step('Verify Kling 2.5 turbo is selected'):
        with step('[Action] verify_phd_str'):
            assert actions.is_element_present(AppiumBy.ACCESSIBILITY_ID, 'Kling 2.5 Turbo')
    with step('Verify 5s and 10s durations are listed'):
        with step('[Action] verify_phd_str'):
            assert actions.is_element_present(AppiumBy.NAME, '5')
        with step('[Action] verify_phd_str'):
            assert actions.is_element_present(AppiumBy.NAME, '10')
    with step('Tap 10s duration'):
        with step('[Action] tap_phd_btn'):
            assert actions.tap_by_locator(AppiumBy.NAME, '10')
    with step('Verify 10s is selected'):
        with step('[Action] verify_phd_str'):
            assert actions.is_element_present(AppiumBy.NAME, '10')
    ...
    with step('Tap outside to close bubble'):
        with step('[Action] tap_outside_bubble'):
            actions.tap_by_coordinates(50, 50)
    ...
    with step('Confirm AI processing notice if present'):
        with step('[Action] tap_phd_btn'):
            if actions.is_element_present(AppiumBy.ACCESSIBILITY_ID, 'AlertDialog-btnPositive', timeout=3):
                assert actions.tap_by_locator(AppiumBy.ACCESSIBILITY_ID, 'AlertDialog-btnPositive')
    ...
    with step("[Verify] test_00167 completion"):
        assert True
