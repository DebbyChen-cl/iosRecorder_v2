...
@pytest.mark.name('00006_main_03_01_06_2_1')
def test_00006_main_03_01_06_2_1(actions: DriverActions):
    ...
    with step('Tap AI issues tab'):
        with step('[Action] tap_ai_issues_tab'):
            assert actions.tap_by_locator(AppiumBy.ACCESSIBILITY_ID, 'AI Issues')
    ...
    with step('Input feedback info'):
        with step('[Action] input_feedback_info'):
            assert actions.type_text_by_locator(AppiumBy.ACCESSIBILITY_ID, 'textViewBackground', 'CLT QA auto test AI issues')
    ...
    with step('Tap What is the issue ? column'):
        with step('[Action] tap_ai_issue_description_field'):
            assert actions.tap_by_locator(AppiumBy.XPATH, '(//XCUIElementTypeTextView[@name="textView"])[2]')
    with step('Input issue description'):
        issue_text = 'CLT QA auto test, please ignore'
        with step('[Action] input_ai_issue_description'):
            assert actions.type_text_by_locator(AppiumBy.XPATH, '(//XCUIElementTypeTextView[@name="textView"])[2]', 'CLT QA auto test, please ignore')
    with step('Verify issue description is displayed'):
        issue_text = 'CLT QA auto test, please ignore'
        with step('[Action] verify_ai_issue_description_displayed'):
            assert actions.verify_text(
                AppiumBy.XPATH,
                '(//XCUIElementTypeTextView[@name="textView"])[2]',
                issue_text,
            ) is not False
    with step('Tap What prompt did you use column'):
        with step('[Action] tap_ai_prompt_description_field'):
            assert actions.tap_by_locator(AppiumBy.XPATH, '(//XCUIElementTypeTextView[@name="textView"])[3]')
    with step('Input prompt description'):
        prompt_text = 'CLT QA auto test prompt'
        with step('[Action] input_ai_prompt_description'):
            assert actions.type_text_by_locator(AppiumBy.XPATH, '(//XCUIElementTypeTextView[@name="textView"])[3]', 'CLT QA auto test prompt')
    with step('Verify prompt description is displayed'):
        prompt_text = 'CLT QA auto test prompt'
        with step('[Action] verify_ai_prompt_description_displayed'):
            assert actions.verify_text(
                AppiumBy.XPATH,
                '(//XCUIElementTypeTextView[@name="textView"])[3]',
                prompt_text,
            ) is not False
        with step('[Action] tap_feedback_tab'):
            actions.tap_by_locator(AppiumBy.ACCESSIBILITY_ID, 'lblTitle')
    with step('Scroll and tap import image button'):
    ...
    with step("[Verify] test_00006 completion"):
        assert True
