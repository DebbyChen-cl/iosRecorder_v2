...
@pytest.mark.name("00196_ai_enhance_advanced_pro_user")
def test_00196_ai_enhance_advanced_pro_user(actions: DriverActions):
    ...
    if not actions.is_element_present(AppiumBy.ACCESSIBILITY_ID, "btnSettings", timeout=3):
        with step("[Action] Recover launcher after an interrupted debug setup"):
            ...
            if (
                not actions.is_element_present(AppiumBy.ACCESSIBILITY_ID, "btnSettings", timeout=1)
                and actions.is_element_present(AppiumBy.ACCESSIBILITY_ID, "btnBack", timeout=1)
            ):
                actions.tap_within_element(AppiumBy.ACCESSIBILITY_ID, "btnBack", 50.0, 50.0)
