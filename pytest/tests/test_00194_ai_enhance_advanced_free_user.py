...
def test_ai_enhance_advanced_free_user(actions: DriverActions):
    ...
    if not actions.is_element_present(AppiumBy.ACCESSIBILITY_ID, "btnSettings", timeout=3):
        with step("[Action] Recover launcher after an interrupted run"):
    ...
    with step("[Action] Set subscription mode to Free"):
        ...
        actions.tap_within_element(AppiumBy.ACCESSIBILITY_ID, "Free", 50.0, 50.0)
        assert actions.verify_visible(AppiumBy.ACCESSIBILITY_ID, "Free")
    ...
    with step("[Verify] Default highlight is on Standard mode"):
        ...
        assert actions.capture_for_preview('ai_enhance_mode_switch', 'before', AppiumBy.ACCESSIBILITY_ID, 'modeSwitcherView')
    ...
    with step("[Verify] Go to IAP page"):
        ...
        assert actions.verify_visible(AppiumBy.ACCESSIBILITY_ID, 'Restore purchase')
