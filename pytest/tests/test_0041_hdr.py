...
@pytest.mark.name("test_hdr")
def test_test_hdr(actions: DriverActions):
    ...
    with step("[Action] Tap HDR"):
        actions.tap_by_locator(AppiumBy.ACCESSIBILITY_ID, 'HDR')
    with step("[Action] Tap Edge"):
        actions.tap_by_locator(AppiumBy.ACCESSIBILITY_ID, 'Edge')
