...
def test_00018_main_G00_01_01(actions: DriverActions):
    ...
    """the code is incorrect, after RD update ID, should record new version!!!!"""
    """credit system"""
    ...
    with step('[Action] perform_task'):
        actions.tap_by_locator(
            AppiumBy.XPATH,
            '//XCUIElementTypeStaticText[@name="taskTitleLabel" and @label="Invite Friends"]',
        )
    ...
    with step('[Action] open_share_artwork'):
        assert actions.tap_by_locator(
            AppiumBy.XPATH,
            '//XCUIElementTypeStaticText[@name="taskTitleLabel" and @label="Share Artwork"]',
        )
    ...
    with step('[Action] perform_task'):
        actions.tap_by_locator(
            AppiumBy.XPATH,
            '//XCUIElementTypeStaticText[@name="taskTitleLabel" and @label="Follow us on Instagram"]',
        )
    ...
    with step('[Action] perform_task'):
        actions.tap_by_locator(
            AppiumBy.XPATH,
            '//XCUIElementTypeStaticText[@name="taskTitleLabel" and @label="Subscribe to Our YouTube"]',
        )
    ...
    with step('[Action] verify_IAP'):
        assert actions.find_element(AppiumBy.ACCESSIBILITY_ID, 'freeTrialLabel')
