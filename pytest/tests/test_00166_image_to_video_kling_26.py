...
@pytest.mark.name('00166_image_to_video_kling_26')
def test_00166_image_to_video_kling_26(actions: DriverActions):
    ...
    with step('Select Kling 2.6'):
        with step('[Action] tap_phd_element'):
            assert actions.tap_by_locator(AppiumBy.ACCESSIBILITY_ID, 'Kling 2.6')
    with step('Verify Kling 2.6 is selected'):
        with step('[Action] verify_phd_str'):
            assert actions.is_element_present(AppiumBy.ACCESSIBILITY_ID, 'Kling 2.6')
    ...
    with step('Verify 10s is selected'):
        with step('[Action] verify_phd_str'):
            assert actions.is_element_present(AppiumBy.NAME, '10')
    with step('Verify only pro quality is listed'):
        with step('[Action] verify_phd_str'):
    ...
    with step("[Verify] test_00166 completion"):
        assert True
