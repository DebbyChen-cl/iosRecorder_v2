...
@pytest.mark.name('00168_image_to_video_kling_o1')
def test_00168_image_to_video_kling_o1(actions: DriverActions):
    ...
    with step('Select Kling O1'):
        with step('[Action] tap_phd_element'):
            assert actions.tap_by_locator(AppiumBy.ACCESSIBILITY_ID, 'Kling O1')
    with step('Verify Kling O1 is selected'):
        with step('[Action] verify_phd_str'):
            assert actions.is_element_present(AppiumBy.ACCESSIBILITY_ID, 'Kling O1')
    ...
    with step("[Verify] test_00168 completion"):
        assert True
