...
@pytest.mark.name("test_main_05_17_02_2")
def test_test_main_05_17_02_2(actions: DriverActions):
    ...
    with step("[Action] Tap Sparkle"):
        actions.tap_by_locator(AppiumBy.ACCESSIBILITY_ID, 'Sparkle')
    with step("[Verify] Tap to apply sparkle style. is not visible"):
        assert actions.verify_not_visible(AppiumBy.NAME, 'Tap to apply sparkle style.'), 'element Tap to apply sparkle style. should not be visible'
    with step("[Action] Tap Manual add"):
        actions.tap_by_locator(AppiumBy.ACCESSIBILITY_ID, 'Manual add')
