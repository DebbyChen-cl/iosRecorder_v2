...
def test_00146_main_05_01_12_7(actions: DriverActions):
    ...
    with step('[Action] close_opening_tutorial'):
        if actions.is_element_present(AppiumBy.ACCESSIBILITY_ID, 'closeButton', timeout=2):
            assert actions.tap_by_locator(AppiumBy.ACCESSIBILITY_ID, 'closeButton', timeout=2)
        if actions.is_element_present(AppiumBy.ACCESSIBILITY_ID, 'btnClose', timeout=2):
            assert actions.tap_by_locator(AppiumBy.ACCESSIBILITY_ID, 'btnClose', timeout=2)
    ...
    with step('[Action] enable_plan_from_settings'):
        assert actions.five_tap_within_element(AppiumBy.ACCESSIBILITY_ID, 'developerButton', 50.0, 50.0)
        ...
        actions.tap_by_locator(AppiumBy.ACCESSIBILITY_ID, 'Perpetual Mode')
    ...
    with step('[Action] wait_process'):
        assert actions.wait_for_visible(AppiumBy.ACCESSIBILITY_ID, 'switchButton', timeout=30)
    ...
    with step('[Action] flip_switch'):
        actions.tap_by_locator(AppiumBy.ACCESSIBILITY_ID, 'switchButton')
    with step('[Action] wait_process'):
        assert actions.wait_for_visible(AppiumBy.NAME, 'Wire Removal', timeout=30)
