...
def test_00044_main_05_04a_01n(actions: DriverActions):
    ...
    if (not actions.tap_by_locator(AppiumBy.ACCESSIBILITY_ID, 'Manual')):
        assert False  # legacy raise
    if (actions.get_text(AppiumBy.XPATH, '//XCUIElementTypeStaticText[@name="valueLabel" and @value="50"]') == '75'):
        pass
    ...
    if (not actions.compare_with_gt('05_04a_01_brushsize_after.png', gt_folder=TD.GT_FOLDER)[0]):
        ...
        assert False  # legacy raise
    with step('[Action] tap_phd_btn'):
        actions.tap_by_locator(AppiumBy.ACCESSIBILITY_ID, 'arrowButton')
    ...
    with step('[Action] tap_edit_home'):
        for __by, __val in [(AppiumBy.ACCESSIBILITY_ID, 'homeButton'), (AppiumBy.ACCESSIBILITY_ID, 'btnHome'), (AppiumBy.ACCESSIBILITY_ID, 'btnHome'), (AppiumBy.IOS_CLASS_CHAIN, '**/XCUIElementTypeWindow/XCUIElementTypeOther[3]/XCUIElementTypeOther/XCUIElementTypeOther[3]/XCUIElementTypeButton[2]'), (AppiumBy.ACCESSIBILITY_ID, 'btn_cancel_n')]:
