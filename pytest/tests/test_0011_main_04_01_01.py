...
@pytest.mark.name("test_main_04_01_01")
def test_test_main_04_01_01(actions: DriverActions):
    ...
    with step("[Verify] btnNight is visible"):
        assert actions.verify_visible(AppiumBy.ACCESSIBILITY_ID, 'btnNight'), 'element btnNight should be visible'
    with step("[Action] Tap btnFlash"):
        actions.tap_by_locator(AppiumBy.ACCESSIBILITY_ID, 'btnFlash')
    with step("[Verify] btnFlash is visible"):
        assert actions.verify_visible(AppiumBy.ACCESSIBILITY_ID, 'btnFlash'), 'element btnFlash should be visible'
    with step("[Action] Tap btnFlash"):
        actions.tap_by_locator(AppiumBy.ACCESSIBILITY_ID, 'btnFlash')
    with step("[Verify] btnFlash is visible"):
        assert actions.verify_visible(AppiumBy.ACCESSIBILITY_ID, 'btnFlash'), 'element btnFlash should be visible'
