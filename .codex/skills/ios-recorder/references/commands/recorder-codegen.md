...
def test_<safe_name>(actions: DriverActions):
    ...
    with step("[Verify] <description>"):
        assert actions.<verify_method>(...), '<verification failure message>'
    assert True
```
...
- **Every verify step contains an explicit assert**: locator-based verifications assert their helper result, screenshot capture steps assert that capture succeeded, and a verify step with no usable locator generates `assert False` instead of silently passing.
- **Screenshot comparison pattern**: `verify_screenshot_gt` / `verify_screenshot_diff` steps assert their inline capture (`capture_for_gt` / `capture_for_preview`); a single `with step("[Verify] Screenshot comparisons"): assert actions.run_screenshot_comparisons() is not False` is appended **once at the end** of the test when any screenshot steps exist — AND logic, all failures raised together
...
- **Verify fallback**: when no element matched, generate `assert False` with the missing-locator context so the verification cannot silently pass.
- **Action fallback `# comment`** when no element matched — non-verify code must always remain valid Python.
