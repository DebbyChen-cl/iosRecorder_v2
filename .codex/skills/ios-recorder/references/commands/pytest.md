...
- Every `[Verify]` and `[Action/Verify]` step must contain an explicit `assert`.
  Verification helpers that return `None` on success, such as `verify_text()` and
  `run_screenshot_comparisons()`, use `assert actions.<method>(...) is not False`.
  Do not rely only on a helper's internal `AssertionError`.
...
- `paint_in_element(by, value, points_pct, duration_ms=1000)` — replay free-form paint path on one element using sampled `(x_pct, y_pct, t_ms)` points
